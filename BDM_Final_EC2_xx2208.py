...
if __name__ == "__main__":
  ...
  placekey_filter = sc.textFile('/tmp/bdm/core-places-nyc') \
          .map(lambda x: next(csv.reader([x]))) \
          .filter(lambda x:x[9][:4] == '4451').map(lambda x:[x[9],x[0]]) \
          .map(lambda x: x[1])\
          .collect()
        
  rdd_first = sc.textFile('weekly-patterns-nyc-2019-2020-sample.csv') \
      .map(lambda x: next(csv.reader([x]))) #read the weekly patterns in sample data
